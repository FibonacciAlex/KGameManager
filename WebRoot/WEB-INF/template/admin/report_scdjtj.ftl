 	...
 	<script type="text/javascript">
        $(function (){
            ...
            var grid = $("#maingrid").ligerGrid({
                ...
                sortName: 'id', 
                columns: [
                 { display: '商店类型', name: 'itemType',width: parseInt(width*0.08),title: '商店类型'},
				 { display: '商店名称', name: 'shopName',width: parseInt(width*0.08),title: '商店名称'},
                { display: '道具编码', name: 'itemCode',width: parseInt(width*0.08),title: '道具的编码'},
                { display: '道具名称', name: 'itemName',width: parseInt(width*0.08),title: '道具的名称'},
                { display: '道具价格（单位：元宝）', name: 'sellPrice',width: parseInt(width*0.1),title: '道具的价格（单位：元宝）'},
                { display: '购买数量', name: 'sellCount',width: parseInt(width*0.08),title: '购买数量'},
                { display: '消费金额（单位：元宝）', name: 'sellTotalPoint',width: parseInt(width*0.1),title: '消费金额（单位：元宝）'},
                { display: '消费占比', name: 'sellRate',width: parseInt(width*0.08),title: '道具的价格'},
                { display: '道具囤积量', name: 'itemStockCount',width: parseInt(width*0.08),title: '道具囤积量'},
				{ display: '拥有道具角色数', name: 'ownerCount',width: parseInt(width*0.08),title: '拥有道具角色数'}
                ],
                width:'100%',
                height:'100%',
                heightDiff:-2,
            ...
            $("#searchButton").click(function(){
            	...
            	form.reportName ='ShopSalesReport';
