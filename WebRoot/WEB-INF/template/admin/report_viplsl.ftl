 	...
 	<script type="text/javascript">
        $(function (){
            ...
            var grid = $("#maingrid").ligerGrid({
                ...
                parms:{
                	reportName:'VIPReport',
                	beginDate:$.date.getFormatDate(beginDate,'yyyyMMdd'),
                	server:1
                },
                sortName: 'id', 
                columns: [
                    { display: 'VIP等级', name: 'vipLevel',width: parseInt(width*0.14)},
	                { display: 'VIP总数', name: 'vipRoleCount',width: parseInt(width*0.14)},
	                { display: '当天登录人数', name: 'loginVipRoleCount',width: parseInt(width*0.14)}
                ],
                width:'100%',
                height:'100%',
                heightDiff:-2,
