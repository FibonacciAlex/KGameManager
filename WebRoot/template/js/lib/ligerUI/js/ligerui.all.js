        ...
        _render: function ()
        {
            ...
            else if (p.url)
            {
                ...
                else
                {
                    ...
                    setTimeout(function ()
                    {
                        g.jiframe.attr("src", p.url+(p.url.indexOf("?")>0?"&":"?")+"framename="+framename);
...
(function ($)
{
    ...
    $.ligerDefaults.GridString = {
        errorMessage: '发生错误',
        pageStatMessage: '显示从{from}条到{to}条，总{total}条 。每页显示：{pagesize}条  ',
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        */
        ...
        _createHeaderCell: function (column)
        {
            ...
            if(column.title){
            	$(".l-grid-hd-cell-text:first", jcell).attr({title:column.title}).addClass("title-tip");
            }
        ...
        _buildPager: function ()
        {
            ...
            if(p.total!=0){
            	...
            	$('.l-bar-text', g.toolbar).html(p.noRecordMessage);
            }
            if (!p.total)
            {
